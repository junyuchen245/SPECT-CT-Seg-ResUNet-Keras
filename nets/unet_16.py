...
def Unet(pretrained_weights = None, input_size = (192,192,1)):
    ...
    conv1_seg = Conv2D(16, 3, activation='relu', padding='same', kernel_initializer='he_normal')(input_seg)
    conv1_seg = BatchNormalization()(conv1_seg)
    conv1_seg = Conv2D(16, 3, activation='relu', padding='same', kernel_initializer='he_normal')(conv1_seg)
    conv1_seg = BatchNormalization()(conv1_seg)
    pool1_seg = MaxPool2D(pool_size=(2, 2))(conv1_seg)

    conv2_seg = Conv2D(32, 3, activation='relu', padding='same', kernel_initializer='he_normal')(pool1_seg)
    conv2_seg = BatchNormalization()(conv2_seg)
    conv2_seg = Conv2D(32, 3, activation='relu', padding='same', kernel_initializer='he_normal')(conv2_seg)
    conv2_seg = BatchNormalization()(conv2_seg)
    pool2_seg = MaxPool2D(pool_size=(2, 2))(conv2_seg)

    conv3_seg = Conv2D(64, 3, activation='relu', padding='same', kernel_initializer='he_normal')(pool2_seg)
    conv3_seg = BatchNormalization()(conv3_seg)
    conv3_seg = Conv2D(64, 3, activation='relu', padding='same', kernel_initializer='he_normal')(conv3_seg)
    conv3_seg = BatchNormalization()(conv3_seg)
    pool3_seg = MaxPool2D(pool_size=(2, 2))(conv3_seg)

    conv4_seg = Conv2D(128, 3, activation='relu', padding='same', kernel_initializer='he_normal')(pool3_seg)
    conv4_seg = BatchNormalization()(conv4_seg)
    conv4_seg = Conv2D(128, 3, activation='relu', padding='same', kernel_initializer='he_normal')(conv4_seg)
    conv4_seg = BatchNormalization()(conv4_seg)
    ...
    conv1_ct = Conv2D(16, 3, activation='relu', padding='same', kernel_initializer='he_normal')(input_ct)
    conv1_ct = BatchNormalization()(conv1_ct)
    conv1_ct = Conv2D(16, 3, activation='relu', padding='same', kernel_initializer='he_normal')(conv1_ct)
    conv1_ct = BatchNormalization()(conv1_ct)
    pool1_ct = MaxPool2D(pool_size=(2, 2))(conv1_ct) #192x192

    conv2_ct = Conv2D(32, 3, activation='relu', padding='same', kernel_initializer='he_normal')(pool1_ct)
    conv2_ct = BatchNormalization()(conv2_ct)
    conv2_ct = Conv2D(32, 3, activation='relu', padding='same', kernel_initializer='he_normal')(conv2_ct)
    conv2_ct = BatchNormalization()(conv2_ct)
    pool2_ct = MaxPool2D(pool_size=(2, 2))(conv2_ct) #96x96

    conv3_ct = Conv2D(64, 3, activation='relu', padding='same', kernel_initializer='he_normal')(pool2_ct)
    conv3_ct = BatchNormalization()(conv3_ct)
    conv3_ct = Conv2D(64, 3, activation='relu', padding='same', kernel_initializer='he_normal')(conv3_ct)
    conv3_ct = BatchNormalization()(conv3_ct)
    pool3_ct = MaxPool2D(pool_size=(2, 2))(conv3_ct) #48x48

    conv4_ct = Conv2D(128, 3, activation='relu', padding='same', kernel_initializer='he_normal')(pool3_ct)
    conv4_ct = BatchNormalization()(conv4_ct)
    conv4_ct = Conv2D(128, 3, activation='relu', padding='same', kernel_initializer='he_normal')(conv4_ct)
    conv4_ct = BatchNormalization()(conv4_ct)
    drop4_ct = Dropout(0.5)(conv4_ct)
    pool4_ct = MaxPool2D(pool_size=(2, 2))(drop4_ct) #24x24

    conv5_ct = Conv2D(256, 3, activation='relu', padding='same', kernel_initializer='he_normal')(pool4_ct)
    conv5_ct = BatchNormalization()(conv5_ct)
    conv5_ct = Conv2D(256, 3, activation='relu', padding='same', kernel_initializer='he_normal')(conv5_ct)
    conv5_ct = BatchNormalization()(conv5_ct)
    conv5_ct = Dropout(0.5)(conv5_ct)
    #pool5_ct = MaxPool2D(pool_size=(2, 2))(conv5_ct) #12x12

    conv5_seg = Conv2D(256, 3, activation='relu', padding='same', kernel_initializer='he_normal')(pool4_seg)
    conv5_seg = BatchNormalization()(conv5_seg)
    conv5_seg = Conv2D(256, 3, activation='relu', padding='same', kernel_initializer='he_normal')(conv5_seg)
    conv5_seg = BatchNormalization()(conv5_seg)
