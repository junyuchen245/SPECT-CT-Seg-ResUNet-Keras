...
class ImageDataGenerator(object):
    ...
    def __init__(self,
        ...
        if data_format == 'channels_first':
            self.channel_axis = 1
            self.size_axis = range(2, self.ndim)
        else:
            self.channel_axis = self.ndim - 1
            self.size_axis = range(1, self.ndim - 1)
    ...
    def random_transform(self, x, seed=None):
        ...
        if np.random.binomial(1, self.augmentation_probability) and np.unique(x).size != 1:
            ...
            if self.rotation_range is not None:
                if np.isscalar(self.rotation_range):
                    ...
                    for rot in self.rotation_range:
                        theta.append(np.pi / 180 * np.random.uniform(-rot, rot) if rot else 0)
            ...
            if self.shift_range is not None:
                ...
                for i, s in enumerate(self.shift_range):
                    shift.append(np.random.uniform(-s, s) * x.shape[img_size_axis[i]] if s else 0)
            ...
            if theta is not None:
                ...
                elif any(th != 0 for th in list(theta)):
                    cd = np.cos(theta[0])
                    sd = np.sin(theta[0])
                    ch = np.cos(theta[1])
                    sh = np.sin(theta[1])
            ...
            # Shift
            if shift is not None and any(sh != 0 for sh in list(shift)):
            ...
            #                             [0, np.cos(shear), 0],
            #                             [0, 0, 1]])
            #     transform_matrix = shear_matrix if transform_matrix is None else np.dot(shear_matrix, transform_matrix)
            ...
            if transform_matrix is not None:
                transform_matrix = transform_matrix_offset_center(transform_matrix, np.array(x.shape)[img_size_axis])
            ...
            if self.flip is not None:
                ...
                for i, fp in enumerate(self.flip):
                    if fp and np.random.random() < 0.5:
                        x = flip_axis(x, img_size_axis[i])  # Warning: this function modifies input array
    ...
    def fit(self, x,
        ...
        if x.ndim not in [4, 5]:
            raise ValueError('Input to `.fit()` should have rank 4 or 5. '
        ...
        if self.featurewise_center:
            self.mean = np.mean(x, axis=[0] + self.size_axis)
            broadcast_shape = np.ones(self.ndim - 1, np.int)
            broadcast_shape[self.channel_axis - 1] = x.shape[self.channel_axis]
            self.mean = np.reshape(self.mean, broadcast_shape)
            x -= self.mean

        if self.featurewise_std_normalization:
            self.std = np.std(x, axis=[0] + self.size_axis)
            broadcast_shape = np.ones(self.ndim - 1, np.int)
            broadcast_shape[self.channel_axis - 1] = x.shape[self.channel_axis]
    ...
    def __init__(self, x, y, image_data_generator,
        ...
        if self.ndim not in [4, 5]:
            raise ValueError('Input data in `NumpyArrayIterator` '
                             'should have rank 4 or 5. You passed an array '
                             'with shape', self.x.shape)

        channels_axis = self.ndim - 1 if data_format == 'channels_last' else 1
